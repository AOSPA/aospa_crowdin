...
def push_as_commit(extracted_files, base_path, project_path, project_name, branch, username):
    ...
    # Push commit
    try:
        repo.git.push(f'ssh://{username}@gerrit.aospa.co:29418/{project_name}',
...
def parse_args():
    ...
    parser.add_argument('-b', '--branch', help='AOSPA Branch',
                        required=True)
...
def upload_sources_crowdin(branch, config_dict, crowdin_path):
    ...
    for cfg in config_dict:
        print(f"\nUploading sources to Crowdin ({cfg})")
        cmd = [
            crowdin_path,
...
def download_crowdin(base_path, branch, xml, username, config_dict, crowdin_path):
    extracted = []
    if config_dict:
        files = [f'{_DIR}/config/{config_dict}']
    else:
        files = [f'{_DIR}/config/{branch}.yaml']
    for cfg in files:
        print(f"\nDownloading translations from Crowdin ({cfg})")
...
def main():
    ...
    base_path_env = f'AOSPA_CROWDIN_BASE_PATH'
    ...
    xml_aospa = load_xml(x=f'{base_path}/manifest/aospa.xml')
    if xml_aospa is None:
        sys.exit(1)

    if args.config:
        config_dict = [f"{_DIR}/config/{args.config}"]
    else:
        config_dict = [f'{_DIR}/config/{default_branch}.yaml']
    if not check_files(config_dict):
        sys.exit(1)
    ...
    if args.download:
        download_crowdin(base_path, default_branch, xml_aospa,
